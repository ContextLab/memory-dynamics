var Fingerprint = function(params) {
    ...
    // if permute is true, correct the fingerprints with permutations
    var permute = params.permute || false;

    // if permute is true, correct the fingerprints with permutations
    var nperms = params.nperms || 1000;
    ...
    function printParams(){
      ...
      console.log('nperms: ' + nperms)

    }

    // compute the fingerprint
    function computeWeights(currentList, recalledWords, callback) {
      ...
      try {
        ...
        if (permute==true){
          var realWeights = _computeFeatureWeights(currentList, recalledWords, features);
          var fakeWeights = Array(nperms).fill(0).map(weight=>{
            return _computeFeatureWeights(currentList, jsPsych.randomization.shuffle(recalledWords), features)
          })

        correctedWeights = {}
        for (feature in realWeights) {
          correctedWeights[feature] =  mean(fakeWeights.map(fakeWeight=>{
            return realWeights[feature] > fakeWeight[feature]
          }))
        }
          callback(correctedWeights)
        } else {
          callback(_computeFeatureWeights(currentList, recalledWords, features));
      }
      } catch (e) {
        console.log('There was an error computing the fingerprint:', e)
        return callback(_defaultFingerprint())
      }
    };
    ...
    var _computeDistance = function(stimArray) {
        ...
        for (var i = 0; i < stimArray.length; i++) {
            for (var j = 0; j < stimArray.length; j++) {
                ...
                // binary distance: stimArray[i].features.firstLetter !== stimArray[j].features.firstLetter ? 1 : 0
                stimArray[i].distances.firstLetter.push({
                    word: stimArray[j].text,
                    dist: Math.abs(alphabet.indexOf(stimArray[i].features.firstLetter) - alphabet.indexOf(stimArray[j].features.firstLetter))
                })
    ...
    function _computeFeatureWeights(curList, recalledWords, features) {
        ...
        for (var i = 0; i < recalledWords.length - 1; i++) {
            ...
            var currentWordIdx = curList.map(function(stimulus) {
                return stimulus.text
            }).indexOf(currentWord);

            var nextWordIdx = curList.map(function(stimulus) {
                return stimulus.text
            }).indexOf(nextWord);
            ...
            if (currentWordIdx !== -1 && nextWordIdx !== -1) {
                ...
                features.forEach(function(feature) {
                    ...
                    var distVec = curList[currentWordIdx].distances[feature];
                    ...
                    listWeights[feature].push(1 - (idxs.reduce(function(a, b) {
                        return a + b
                    }, 0) / idxs.length) / filteredDistVec.length);
