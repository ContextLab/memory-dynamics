...
# default arg values
IMAGE_NAME="memory-dynamics"                             # name of image to build/use
CONTAINER_NAME="MD"                                      # name of container to create/start
declare -i LAUNCH_BROWSER=1                              # true by default
declare -i DETACH=0                                      # false by default
declare -i DEVELOP=0                                     # false by default
...
get_nbserver_url() {
    ...
    running_nbserver=$(docker exec "$CONTAINER_NAME" bash -c 'jupyter nbclassic list')
